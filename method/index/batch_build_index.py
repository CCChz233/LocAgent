...
def run(rank: int, repo_queue, args, gpu_ids: list, total_repos: int = 0):
            ...
            if graph_index_dir:
                ...
                if osp.exists(graph_index_file):
                    try:
                        ...
                        if span_ids_map:
                            print(f'[GPU {device}] Extracted span_ids for {len(span_ids_map)}/{len(blocks)} blocks in {repo_name}')
                    except Exception as e:
                        print(f'[GPU {device}] Warning: Failed to extract span_ids for {repo_name}: {e}')
    ...
    pbar.close()
    print(f'[GPU {device}] Done. Processed: {processed}, Skipped: {skipped}, Failed: {failed}')
...
def main():
    ...
    parser.add_argument("--split", type=str, default="test",
                        help="数据集分片")
    parser.add_argument("--repo_path", type=str, default="playground/locbench_repos",
                        help="本地仓库根目录")
    ...
    parser.add_argument(
        "--strategy",
        choices=["fixed", "sliding", "rl_fixed", "rl_mini", "epic", "function_level", 
